...
library(neuralnet)
ann <- neuralnet(formula = V42~PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9 ,data = train_pca , hidden = c(7,7),learningrate = NULL, lifesign = "full", threshold = 0.05, linear.output = FALSE, err.fct = "sse",lifesign.step = 1000,stepmax = 1e+05,algorithm = "rprop+",rep=1)


##Predicting the Test set results
y_pred = compute(ann, newdata = (test_pca[-10]))
y_pred = (y_pred > 0.5)
